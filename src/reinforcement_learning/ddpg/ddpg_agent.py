...
from reinforcement_learning.ddpg.ddpg_nn_models import CriticNetwork2D, ActorNetwork2D
from reinforcement_learning.ddpg.ou_noise import OUNoiseGenerator
...
class DDPGAgent(RLAgent):
    def __init__(self, state_dim, action_dim, action_low, action_high, gamma, tau, buffer_size,
                 critic_lr=1e-3, actor_lr=1e-3, force_cpu=False):
        super().__init__()
    ...
    def train_step(self, batch_size):
        ...
        state_batch, action_batch, reward_batch, next_state_batch, _ = self.replay_buffer_.sample_random_batch(batch_size)
